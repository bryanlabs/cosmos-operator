...
RUN export  CGO_ENABLED=1 \
    ...
    go build -tags 'rocksdb pebbledb' -ldflags "-X github.com/bryanlabs/cosmos-operator/internal/version.version=$VERSION $LDFLAGS" -a -o manager .

# Build final image from scratch
FROM scratch

LABEL org.opencontainers.image.source=https://github.com/bryanlabs/cosmos-operator
